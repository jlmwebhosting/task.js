...
function pick_enqueue() {
	console.log(global && global.process && process.nextTick);
	return (global && global.process && process.nextTick) ?
		enqueue_nextTick() :
		(typeof window.postMessage === "function") ?
		enqueue_postMessage() :
		enqueue_setTimeout();
}
var enqueue = pick_enqueue();

var supportsLegacyGenerators = false;
try {
    Function("(function(){ yield 0 })");
    supportsLegacyGenerators = true;
} catch (e) {}


// FIXME: The current V8 generator implementation still uses "send" while the
// SpiderMonkey implementation uses the more up to date "next". Remove this
// when V8 updates.
var sendMethod = "next";
try {
    if ("send" in Function("return function*(){}()")())
        sendMethod = "send";
} catch (e) {}
...
function handleResult(task, promise) {
...
function runScheduledTask(task) {
    ...
    if (task.threadState === T_CANCELLED) {
        ...
        task.runState = R_RESOLVED;
        task.threadState = T_CLOSED;
    } else if (!supportsLegacyGenerators || !("send" in task.thread)) {
        try {
            var ret = send ? task.thread[sendMethod](result) : task.thread["throw"](result);
            if (ret.done)
                resolveTask(task, ret.value);
            else
                handleResult(task, ret.value || READY);
        } catch (e) {
            rejectTask(task, e);
        }
    } else {
        try {
            var p = send ? task.thread.send(result) : task.thread["throw"](result);
            handleResult(task, p || READY);
        } catch (e) {
            if (e instanceof TaskResult || e instanceof StopIteration)
                resolveTask(task, e.value);
            else
                rejectTask(task, e);
        }
    }
